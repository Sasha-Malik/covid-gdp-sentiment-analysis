...
library(brms)
dv <- brm(formula = gdp_2020_values~avg_score_2020, data=df3, family=gaussian ,refresh=0)
...
attach(df3)
uncertainty_coef<-as_draws_df(dv, variable = "b_avg_score_2020")
ggplot(uncertainty_coef, aes(x=b_avg_score_2020))+geom_density()+labs(title= "Uncertainty in regression coefficient",
...
```{r plot_effects}
conditional_effects(dv,effects="avg_score_2020")
```
